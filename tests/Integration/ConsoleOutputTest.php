...
it('shows version details only for the version option', function (): void {
    $tester = new ApplicationTester(makeConsoleOutputApplication());

    $status = $tester->run(['--version' => true], ['decorated' => false]);
    $display = $tester->getDisplay();

    expect($status)->toBe(Command::SUCCESS)
        ->and($display)->toContain('DeployCore')
        ->and($display)->toContain('Ver:')
...
it('shows env and inventory details only at verbose command output levels', function (): void {
