...
#   DEPLOY_REPO_PATH     - Path to the git cache (bare) repository
#   DEPLOY_DOMAIN        - Site domain (example.com)
#   DEPLOY_BRANCH        - Git branch being deployed
#   DEPLOY_PHP_VERSION   - PHP version selected for this site (e.g. 8.4)
#   DEPLOY_PHP           - Absolute path to the PHP binary (e.g. /usr/bin/php8.4)
#
# You're automatically in the DEPLOY_RELEASE_PATH directory at this point:
...
# `deployer`. In DeployCore setups, `www-data` is added to the
...
set_group_writable_tree() {
	...
	# deploy.sh runs via sudo so chmod succeeds on all entries regardless of
	# ownership (deployer or www-data).
...
if [[ $framework == "laravel" ]]; then
	...
	mkdir -p "${DEPLOY_SHARED_PATH}/storage/"{app,framework,logs}
	mkdir -p "${DEPLOY_SHARED_PATH}/storage/framework/"{cache,sessions,views}

	# Make shared storage writable for both deploy user and PHP-FPM group member:
	set_group_writable_tree "${DEPLOY_SHARED_PATH}/storage"

	echo "→ Linking shared storage..."
	rm -rf "${DEPLOY_RELEASE_PATH}/storage"
	ln -sfn "${DEPLOY_SHARED_PATH}/storage" "${DEPLOY_RELEASE_PATH}/storage"

	echo "→ Ensuring shared sqlite database..."
	mkdir -p "${DEPLOY_SHARED_PATH}/database"
	touch "${DEPLOY_SHARED_PATH}/database/database.sqlite"

	# Keep sqlite path writable for both deploy user and PHP-FPM group member:
	chmod 2775 "${DEPLOY_SHARED_PATH}/database"
	chmod 664 "${DEPLOY_SHARED_PATH}/database/database.sqlite"

	echo "→ Linking shared database.sqlite..."
	ln -sf "${DEPLOY_SHARED_PATH}/database/database.sqlite" "${DEPLOY_RELEASE_PATH}/database/database.sqlite"
fi

# Symfony (uncomment as needed)
# if [[ $framework == "symfony" ]]; then
# 	echo "→ Ensuring shared var directories..."
# 	mkdir -p "${DEPLOY_SHARED_PATH}/var/"{log,sessions}
#
# 	echo "→ Linking shared var..."
# 	rm -rf "${DEPLOY_RELEASE_PATH}/var"
# 	ln -sfn "${DEPLOY_SHARED_PATH}/var" "${DEPLOY_RELEASE_PATH}/var"
# fi

# CodeIgniter (uncomment as needed)
# if [[ $framework == "codeigniter" ]]; then
# 	echo "→ Ensuring shared writable directories..."
# 	mkdir -p "${DEPLOY_SHARED_PATH}/writable/"{cache,logs,session,uploads}
#
# 	echo "→ Linking shared writable..."
# 	rm -rf "${DEPLOY_RELEASE_PATH}/writable"
# 	ln -sfn "${DEPLOY_SHARED_PATH}/writable" "${DEPLOY_RELEASE_PATH}/writable"
# fi

# .env (framework-agnostic)
if [[ -f "${DEPLOY_SHARED_PATH}/.env" ]]; then
	echo "→ Linking shared .env..."
	ln -sf "${DEPLOY_SHARED_PATH}/.env" "${DEPLOY_RELEASE_PATH}/.env"
fi
...
if [[ -f composer.json ]]; then
	...
	"${DEPLOY_PHP}" "${composer_bin}" install --no-interaction --no-dev --optimize-autoloader
fi
...
if [[ $framework == "laravel" ]]; then
	...
	"${DEPLOY_PHP}" artisan storage:link

	echo "→ Ensuring app key exists..."
	"${DEPLOY_PHP}" artisan key:generate || true
...
# 	# Run migrations
# 	echo "→ Running migrations..."
# 	"${DEPLOY_PHP}" bin/console doctrine:migrations:migrate --no-interaction
#
# 	# Code caching
# 	echo "→ Clearing cache..."
# 	"${DEPLOY_PHP}" bin/console cache:clear
# fi
...
# 	# Run migrations
# 	echo "→ Running migrations..."
# 	"${DEPLOY_PHP}" spark migrate --all
#
# 	# Code caching
# 	echo "→ Optimizing..."
# 	"${DEPLOY_PHP}" spark optimize
# fi
