...
namespace DeployCore\Console\Php;

use DeployCore\Contracts\BaseCommand;
use DeployCore\Traits\PhpTrait;
...
#[AsCommand(
    name: 'php:start',
    description: 'Start PHP-FPM service(s)'
)]
class PhpStartCommand extends BaseCommand
{
    use PhpTrait;
    use PlaybooksTrait;
    use ServersTrait;
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $result = $this->executePlaybookSilently(
            $server,
            'php-service',
            "Starting {$versionDisplay}...",
            [
                'DEPLOY_ACTION' => 'start',
                'DEPLOY_PHP_VERSIONS' => $versionList,
            ],
        );

        if (is_int($result)) {
            $this->nay("Failed to start {$versionDisplay}");

            return Command::FAILURE;
        }

        $this->yay("{$versionDisplay} started");
