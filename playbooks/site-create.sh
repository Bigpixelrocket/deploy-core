...
# DEPLOY_WEB_ROOT can be empty (valid for WordPress/root-serving apps)
...
#
# Returns the web root path (handles empty DEPLOY_WEB_ROOT)
...
get_web_root_path() {
	...
	if [[ -z $DEPLOY_WEB_ROOT ]]; then
		...
		echo "${site_path}/current/${DEPLOY_WEB_ROOT}"
	fi
}
...
configure_nginx_vhost() {
	...
	local php_version=$DEPLOY_PHP_VERSION
...
${server_block}
			...
			;;
		none)
			# No www alias (single host only)
			nginx_config="# Site: ${domain} (No www alias)
# Generated by DeployCore
...
${server_block}
			...
			;;
		*)
			echo "Error: Invalid WWW mode: ${www_mode}. Expected: redirect-to-root, redirect-to-www, or none" >&2
			exit 1
			;;
	esac
